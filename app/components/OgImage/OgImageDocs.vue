...
<template>
  <div
    class="w-full h-full flex flex-col justify-center py-16 bg-default min-h-120"
  >
    <svg
    ...
    <div class="px-25 flex flex-col h-full">
      <div class="mb-10">
        <AppLogo />
      </div>
      ...
      <p
        v-if="headline"
        class="uppercase mb-4 font-semibold bg-primary text-black w-max px-2"
      >
        {{ headline }}
      </p>
      <h1
        v-if="title"
        class="m-0 text-5xl font-semibold mb-4 flex items-center"
      >
        <span>{{ title }}</span>
      </h1>
      <p v-if="description" class="text-xl text-muted leading-tight w-175">
        {{ description }}
      </p>

      <div class="flex flex-wrap justify-end gap-1 mt-auto pt-16">
        <UButton
